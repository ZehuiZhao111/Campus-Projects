...
do_join(State, Ref, ChatName) ->
    CharRooms = State#cl_st.con_ch,
    case maps:is_key(ChatName,CharRooms) of 
        false -> whereis(server)!{self(),Ref,join, ChatName},
                receive
                    {From, _Ref, connect, History} -> {History,State#cl_st{con_ch=maps:put(ChatName,From,CharRooms)}};
                    _ -> {err,State}
                end;
        true -> {err,State}
    end.
